...
start_recovery = start + ford/2 + start + 1;
recovery_signal = recover_signal(start_recovery:mp:end);
figure();
pwelch(recovery_signal,500,300,500,Fs,'power');
title('PSD bit stream');
%eyediagram(recover_signal(1:Fs), 3*mp);
